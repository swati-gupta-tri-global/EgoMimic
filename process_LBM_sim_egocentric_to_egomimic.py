...
def process_raw_data(inpt):
    ...
    with h5py.File(hdf5_path, "w") as f:
        ...
        for ep_idx, episode in tqdm(enumerate(episodes_list), total=len(episodes_list), desc=f"process"):
            ...
            language_instructions = load_yaml("/home/ubuntu/lbm/diffusion_policy/config/task/language_annotations.yaml")["language_dict"]
            ...
            if not os.path.isfile(meta_data_file):
                print(f"{meta_data_file} does not exist. Skipping. raw_episode_path: {raw_episode_path}.") 
            else:
    ...
    split_train_val_from_hdf5(hdf5_path=hdf5_path, val_ratio=0.2)
