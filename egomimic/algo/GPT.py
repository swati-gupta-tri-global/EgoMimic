...
class SpatialSoftmax(torch.nn.Module):
    def __init__(self, in_c, in_h, in_w, num_kp=None):
        ...
        pos_x, pos_y = torch.meshgrid(
            torch.from_numpy(np.linspace(-1, 1, in_w)).float(),
    ...
    def forward(self, x):
        ...
        keypoint_x = torch.sum(self.pos_x * attention, dim=1, keepdim=True).view(
            -1, self._num_kp
        )
        keypoint_y = torch.sum(self.pos_y * attention, dim=1, keepdim=True).view(
            -1, self._num_kp
        )
...
class GPT_wrapper_scratch(nn.Module):
    ...
    def __init__(
        ...
        self.ee_image_enc = torch.nn.Sequential(
    ...
    def build_grid(self, source_size, target_size):
        ...
        direct = (
            torch.linspace(-k, k, target_size)
            .unsqueeze(0)
    ...
    def random_crop_grid(self, x, grid):
        ...
        grid[:, :, :, 0] = grid[:, :, :, 0] + torch.FloatTensor(
            x.size(0)
        ).cuda().random_(0, delta).unsqueeze(-1).unsqueeze(-1).expand(
            -1, grid.size(1), grid.size(2)
        ) / x.size(
            2
        )
        # Add random shifts by y
        grid[:, :, :, 1] = grid[:, :, :, 1] + torch.FloatTensor(
            x.size(0)
        ).cuda().random_(0, delta).unsqueeze(-1).unsqueeze(-1).expand(
            -1, grid.size(1), grid.size(2)
        ) / x.size(
    ...
    def forward_train(self, obs, goal_img):
        ...
        x_goal_image = F.grid_sample(
            x_goal_image, x_goal_grid_shifted, align_corners=True
        )
        ...
        x_pose = torch.cat(
            (obs["robot0_eef_pos"], obs["robot0_eef_quat"]), dim=-1
        ).contiguous()
        x_pose_feat = self.mlp_encoder_pose(x_pose)

        input_tensor = torch.cat(
            (x_feature, x_goal_feature, x_ee_image, x_pose_feat), dim=-1
        ).contiguous()
        ...
        for i in range(N):
            ...
            idx_cond = (
                idx
                if idx.size(1) <= self.gpt_model.block_size
                else idx[:, -self.gpt_model.block_size :]
            )
            # forward the model to get the logits for the index in the sequence
...
class GPT_wrapper(nn.Module):
    ...
    def forward_train(self, obs):
        ...
        input_tensor = torch.cat(
            (x_feature, x_ee_image, x_pose_feat), dim=-1
        ).contiguous()
