...
def prep_for_mimicplay(hdf5_path, data_type):
    ...
    for demo_key in demo_keys:
        h5py_file[f"data/{demo_key}"].attrs["num_samples"] = h5py_file[
            f"data/{demo_key}/actions"
        ].shape[0]
...
def normalize_obs(h5py_file):
    ...
    for demo_key in demo_keys:
        ...
        ee_pose_unnorm[:, 0] = general_norm(
            ee_pose_unnorm[:, 0], -1, 1, arr_min=mins[0], arr_max=maxs[0]
        )
        ee_pose_unnorm[:, 1] = general_norm(
            ee_pose_unnorm[:, 1], -1, 1, arr_min=mins[1], arr_max=maxs[1]
        )
        ee_pose_unnorm[:, 2] = general_norm(
            ee_pose_unnorm[:, 2], -1, 1, arr_min=mins[2], arr_max=maxs[2]
        )
        ...
        h5py_file[f"data/{demo_key}/obs/ee_pose_unnorm"] = h5py_file[
            f"data/{demo_key}/obs/ee_pose"
        ]
        del h5py_file[f"data/{demo_key}/obs/ee_pose"]
        h5py_file[f"data/{demo_key}/obs/ee_pose"] = h5py_file[
            f"data/{demo_key}/obs/ee_pose_norm"
        ]
...
def chunk_actions(h5py_file, POINT_GAP=15, FUTURE_POINTS_COUNT=10):
    ...
    for demo_key in demo_keys:
        ...
        future_traj_data = np.array(
            [
                get_future_points(eef_pos[j:], POINT_GAP, FUTURE_POINTS_COUNT)
...
def remove_eepose_quat(h5py_file):
    ...
    for demo_key in demo_keys:
        h5py_file[f"data/{demo_key}/obs/ee_pose_full_unnorm"] = h5py_file[
            f"data/{demo_key}/obs/ee_pose"
        ]
        del h5py_file[f"data/{demo_key}/obs/ee_pose"]
        h5py_file[f"data/{demo_key}/obs/ee_pose"] = h5py_file[
            f"data/{demo_key}/obs/ee_pose_full_unnorm"
        ][:, :3]
...
def base_to_cam(h5py_file, T_cam_base):
    ...
    for demo_key in demo_keys:
        h5py_file[f"data/{demo_key}/obs/ee_pose_cam_frame"] = ee_pose_to_cam_frame(
            h5py_file[f"data/{demo_key}/obs/ee_pose"][:], T_cam_base
        )[:, :3]
        del h5py_file[f"data/{demo_key}/obs/ee_pose"]
        h5py_file[f"data/{demo_key}/obs/ee_pose"] = h5py_file[
            f"data/{demo_key}/obs/ee_pose_cam_frame"
        ]
    ...
    for demo_key in demo_keys:
        ...
        try:
            h5py_file.create_dataset(
                f"data/{demo_key}/obs/front_img_1",
                data=filtered_front_img_1,
                chunks=chunks,
            )
            h5py_file.create_dataset(
                f"data/{demo_key}/obs/ee_pose", data=filtered_ee_pose
            )
