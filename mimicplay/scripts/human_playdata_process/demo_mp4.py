...
def parse_args():
    ...
    parser.add_argument(
        "--left_video", help="left view video", default="vis_1.mp4", type=str
    )
    parser.add_argument(
        "--right_video", help="right view video", default="vis_2.mp4", type=str
    )
    parser.add_argument(
        "--dataset",
        dest="dataset",
    ...
    parser.add_argument(
        "--hdf5_path", help="h5py file path", default="demo_hand_loc_0.hdf5", type=str
    )
    parser.add_argument(
        "--target_hdf5_path",
        help="target h5py file path",
        default="demo_hand_loc_1.hdf5",
        type=str,
    )
    parser.add_argument(
        "--hdf5_read_mode", help="online / offline", default="online", type=str
    )
    parser.add_argument(
        "--cfg",
        dest="cfg_file",
    ...
    parser.add_argument(
        ...
        default=1,
        type=int,
        help="set to n: sample 1 frame every n frames",
    )
...
def export_images_from_hdf5(
    hdf5_path="", target_path="images", demo_name="demo_1", front_image_index=None
):
    ...
    for demo_index in h5py_file.keys():
        ...
        for img_idx in range(front_image.shape[0]):
            ...
            cv2.imwrite(
                os.path.join(target_path, "{:06d}.png".format(img_idx)), bgr_img
            )
...
if __name__ == "__main__":
    ...
    for front_image_index in front_image_indexes:
        ...
        for demo_name in demo_name_list:
            ...
            mpy_frame_list = []
            print(
                "Processing {} front_image_index {} ...".format(
                    demo_name, front_image_index
                )
            )
            if args.hdf5_path is not None and args.hdf5_read_mode == "offline":
                export_images_from_hdf5(
                    args.hdf5_path,
                    demo_name=demo_name,
                    front_image_index=front_image_index,
                )
            ...
            with torch.no_grad():
                ...
                while num_images > 0:
                    ...
                    if hand_dets is not None:
                        ...
                        save_hand_dets = np.concatenate(
                            (
                                save_hand_dets,
                                np.array(calculate_center(save_hand_dets[0, :4]))[None],
                            ),
                            axis=1,
                        )
                        save_hand_dets[:, :4] = (
                            save_hand_dets[:, :4].astype(np.float32) / img_size
                        )
                        save_hand_dets[:, -2:] = (
                            save_hand_dets[:, -2:].astype(np.float32) / img_size
                        )
                    ...
                    if vis:
                        result_path = os.path.join(
                            demo_dir,
                            imglist[num_images][:-4]
                            + "_{}".format(front_image_index)
                            + "_det.png",
                        )
                ...
                print(
                    "hand_loc.shape: ", hand_loc.shape
                )  # hand_loc.shape:  (145, 1, 12)
                shifted_hand_loc_1_array = np.concatenate(
                    (hand_loc, hand_loc[-1:]), axis=0
                )
                hand_act = shifted_hand_loc_1_array[1:] - shifted_hand_loc_1_array[:-1]
                print(
                    "hand_act.shape: ", hand_act.shape
                )  # hand_act.shape:  (145, 1, 12)

                obs_path = "data/" + demo_name + "/obs"
                new_f_out.create_dataset(
                    obs_path + "/hand_loc_{}".format(front_image_index), data=hand_loc
                )
                new_f_out.create_dataset(
                    obs_path + "/hand_act_{}".format(front_image_index), data=hand_act
                )
                new_f_out.create_dataset(
                    obs_path + "/front_image_{}".format(front_image_index), data=image
                )
                ...
                if front_image_index == 1:
                    ...
                    all_hand_loc = np.concatenate(
                        (hand_loc_1[:, :, -2:], hand_loc[:, :, -2:]), axis=2
                    )
                    new_f_out.create_dataset(obs_path + "/hand_loc", data=all_hand_loc)
                    print(
                        "all_hand_loc.shape: ", all_hand_loc.shape
                    )  # hand_act.shape:  (145, 1, 4)
                    all_skip_hand_loc = []
                    num_future_frame = 10
                    skip_len = 2
                    ...
                    for i in range(all_hand_loc.shape[0]):
                        ...
                        for j in range(num_future_frame):
                            if i + j * skip_len >= all_hand_loc.shape[0]:
                                ...
                                each_skip_hand_loc.append(
                                    all_hand_loc[i + j * skip_len]
                                )
                    ...
                    new_f_out.create_dataset(
                        "data/" + demo_name + "/actions", data=all_skip_hand_loc
                    )
                    # 'actions', 'dones', 'interventions', 'next_obs', 'obs', 'policy_acting', 'rewards', 'states', 'user_acting'
                    new_f_out.create_dataset(
                        "data/" + demo_name + "/dones", data=np.zeros((T - 1))
                    )
                    new_f_out.create_dataset(
                        "data/" + demo_name + "/interventions", data=np.zeros((T, 1))
                    )
                    new_f_out.create_dataset(
                        "data/" + demo_name + "/policy_acting", data=np.zeros((T))
                    )
                    new_f_out.create_dataset(
                        "data/" + demo_name + "/rewards", data=np.zeros((T - 1))
                    )
                    new_f_out.create_dataset(
                        "data/" + demo_name + "/states", data=np.zeros((0))
                    )
                    new_f_out.create_dataset(
                        "data/" + demo_name + "/user_acting", data=np.zeros((T, 1))
                    )
