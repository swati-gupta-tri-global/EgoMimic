...
def process_dataset(dataset_file):
    ...
    with h5py.File(dataset_file, "r+") as f:
        ...
        for i in range(0, DEMO_COUNT):
            ...
            f.create_dataset(
                f"data/demo_{i}/obs/robot0_eef_pos_future_traj", data=future_traj_data
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process hdf5 dataset to generate future trajectory data."
    )
    parser.add_argument(
        "--dataset", required=True, help="Path to the hdf5 dataset file."
    )
