...
def render_traj(img, cam2base, K, trajs, colors):
    for i in range(len(trajs)):
        ...
        img = draw_dot_on_frame(img, traj_pixels, palette=colors[i], dot_size=2, show=False)
...
def render_trajs_batch(img_data, traj_dict, cam2base, K, colors):
    ...
    for i in range(b):
        ...
        img = np.ascontiguousarray(np.array(img_data[i, 0]*255, dtype="uint8").transpose(1, 2, 0))
        img = render_traj(
            img,
            cam2base=cam2base,
            K=K,
...
def resize_curr_img(curr_image):
    ...
    curr_image = TVF.resize(curr_image, size=(240, 320), interpolation=TVF.InterpolationMode.BILINEAR)
    orig_shape[-2] = 240
    orig_shape[-1] = 320
